...
import { ThumbsUp, ThumbsDown, Minus, Share2, Twitter, Linkedin, Copy, Check, Gift } from 'lucide-react';
...
const SuccessFeedback: React.FC<SuccessFeedbackProps> = ({
  ...
  const handleShare = async (method: 'twitter' | 'linkedin' | 'copy') => {
    try {
      ...
      if (error.details?.alreadyClaimed) {
        toast('Bonus already claimed for this query', { icon: 'ℹ' });
      } else if (error.details?.dailyLimitReached) {
        toast('Daily share bonus limit reached', { icon: '' });
      } else {
  ...
  if (showShareOptions) {
    return (
      <div className={`p-4 rounded-xl bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 ${className}`}>
        ...
        {!bonusEarned && (
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 bg-[#1DA1F2]/20 hover:bg-[#1DA1F2]/30 text-[#1DA1F2] rounded-lg transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 bg-[#0A66C2]/20 hover:bg-[#0A66C2]/30 text-[#0A66C2] rounded-lg transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 bg-slate-700 hover:bg-slate-600 text-gray-300 rounded-lg transition-colors"
            >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all ${
            feedback === 'yes'
              ...
              : 'bg-slate-800 hover:bg-slate-700 text-gray-400 hover:text-green-400'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all ${
            feedback === 'partial'
              ...
              : 'bg-slate-800 hover:bg-slate-700 text-gray-400 hover:text-amber-400'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all ${
            feedback === 'no'
              ...
              : 'bg-slate-800 hover:bg-slate-700 text-gray-400 hover:text-red-400'
          }`}
        >
